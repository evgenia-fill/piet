...
def get_next_position_in_block(block):
    return min(block)
...
class PixelInterpreter:
    ...
    def get_command(self, curr_color, next_color):
        ...
        return colour_dict.get((hue, lightness))
        ...
        try:
            if command == 'push':
                ...
                if depth <= 0 or depth > len(self.stack):
                    return
                slice = self.stack[-depth:]
                rolls = rolls % depth
                self.stack[-depth:] = slice[-rolls:] + slice[:-rolls]
